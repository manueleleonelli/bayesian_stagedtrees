...
bb <- list(c(1,1),c(1,1))
cc <- list(c(1,1),c(1,1))
ff <- list(c(0.2^2/0.2,0.2^2/0.2),c(0.2^2/0.2,0.2^2,0.2^0.2))
gg <- list(c(0.2/0.2,0.2/0.2),c(0.2/0.2,0.2/0.2))
...
data$Age <- scale(sample(18:60, sample_size, replace = T))
data$Income <- scale(rnorm(sample_size, 30, sd =5 ))




mcmc_crp_distances <- function(tree, data, n_save, n_burn = 0, thin = 1, a = 1, kappa = 1, csi = 0 , scope = NULL, update_SM = TRUE, update_CRP= TRUE){
  ...
  gamma_out <- vector("list", length = n_save)
  lambda_out <- vector("list", length = n_save)
  pi_out <- vector("list", length = n_save)
  prop_var_out <- vector("list", length = n_save)
  
  ### 
  pi_current <- lapply(1:n_v, function(x) runif(n_nums, min = 0, max = 1))
  gamma_current <- lapply(1:n_v, function(x) rbinom(n_nums, 1, 0.5))
  ...
  for(it in 1:n_tot){
    ...
    for(iv in 1:n_v){
      ...
      pi_current_iv <- pi_current[[iv]]
      lambda_current_iv <- lambda_current[[iv]]
      gamma_current_iv <- gamma_current[[iv]]
      lamb_iv <- lambda_current_iv*gamma_current_iv
      ...
      if(update_CONT){
        ...
        gamma_proposal_iv <- gamma_current_iv
        ...
        for(i in 1:n_nums){
          ...
          gamma_proposal_iv[i] <- rbinom(1,1,pi_current_iv[i])
          if(gamma_proposal_iv[i] !=0) {
            count_gamma[[iv]][i] <- count_gamma[[iv]][i] + 1
            lambda_proposal_iv[i] <- rgamma(1, shape = lambda_current_iv[i]^2/prop_var[[iv]][i], scale= prop_var[[iv]][i]/lambda_current_iv[i])
            while(lambda_proposal_iv[i] <= 0.000000001) {lambda_proposal_iv[i] <- rgamma(1, shape = lambda_current_iv[i]^2/prop_var[[iv]][i], scale= prop_var[[iv]][i]/lambda_current_iv[i])}
            }

          # MINIBATCH PARTIZIONE AUSILIARIA
          rho_prime <- sample_partition(c_ij, kappa_iv, csi_iv, gamma_proposal_iv[i]*lambda_proposal_iv[i], iterations = 100) 
            
          ## Quantita MH prior lambda
          num_lambda <- dgamma(lambda_proposal_iv[i], shape = 0.4^2/0.1, scale = 0.1/0.4,log =T)
          #(ff_iv[i]-1)*log(lambda_proposal_iv[i]) - gg_iv[i]*lambda_proposal_iv[i]
          den_lambda <- dgamma(lambda_current_iv[i], shape = 0.4^2/0.1, scale = 0.1/0.4,log =T)
          #(ff_iv[i]-1)*log(lambda_current_iv[i]) - gg_iv[i]*lambda_current_iv[i]
          
          # Quantita MH prior gamma
          num_gamma <- gamma_proposal_iv[i]*log(pi_current_iv[i]) + (1-gamma_proposal_iv[i])*log(1-pi_current_iv[i])
          den_gamma <- gamma_current_iv[i]*log(pi_current_iv[i]) + (1-gamma_current_iv[i])*log(1-pi_current_iv[i])
          ...
          for(k in 1:num_part1){
            ...
            num_dist1 <- num_dist1 - gamma_proposal_iv[i]*lambda_proposal_iv[i]*sum(mat1)/2
            den_dist1 <- den_dist1 - gamma_current_iv[i]*lambda_current_iv[i]*sum(mat1)/2
          }
          ...
          for(k in 1:num_part2){
            ...
            num_dist2 <- num_dist2 -gamma_current_iv[i]*lambda_current_iv[i]*sum(mat_2)/2
            den_dist2 <- den_dist2 - gamma_proposal_iv[i]*lambda_proposal_iv[i]*sum(mat_2)/2
          }
        ...
        prob <- exp(num_lambda+num_gamma+num_trans+num_dist1+num_dist2- den_lambda-den_gamma-den_trans-den_dist1-den_dist2)
        if( runif(1) < prob ){
          gamma_current_iv <- gamma_proposal_iv
        ...
        gamma_current[[iv]] <- gamma_current_iv
        
        ### SAMPLE PI's
        pi_current[[iv]] <- rbeta(n_nums,bb_iv +gamma_current_iv, cc_iv + (1-gamma_current_iv))
      
        ## ADAPTIVE VARIANCE
        indices <- which(count_gamma[[iv]] == 50)
    ...
    if((it > n_burn) & ((it - n_burn)%%thin == 0)){
      ...
      pi_out[[iter]] <- pi_current
      lambda_out[[iter]] <- lambda_current
      gamma_out[[iter]] <- gamma_current
    }
  ...
  OUTPUT_MCMC <- list("alloc_v_out" = alloc_v_out, "nj_v_out" = nj_v_out, "K_v_out" = K_v_out, "chain_out" = chain_out, "pi_out" = pi_out, "lambda_out" = lambda_out, "gamma_out"=gamma_out, "prop_var_out" = prop_var_out)
  return(OUTPUT_MCMC)
}
