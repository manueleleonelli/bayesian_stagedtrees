mcmc_crp_gamma_no_hamming <- function(tree, data, n_save, n_burn = 0, thin = 1, a = 1, kappa = 1, csi = 0.25 , scope = NULL, update_SM = TRUE, update_CRP= TRUE){
    ...
    #  dist_H_iv <- dist_H[[iv]]
      ...
      #    dist_H_j <- rep(0,K_iv)
         ...
         #   dist_H_j[k] <- sum(dist_H_iv[i,ind_k])
          ...
          w <- c(kappa_iv, nj_iv * exp( - Reduce(`+`, Map(function(x, y) x * y, dist_nums_j, lamb_iv)) ))
      ...
      if(update_SM){
       ...
       #   dist_b <- sum(dist_H_iv[ixb,ixb])/2
        ...
        #  dist_SM <- sum(dist_H_iv[ix_SM,ix_SM])/2
          ...
          log_ar_SM <- log_ar_SM  - sum(lamb_iv*(dist_SM_cont-dist_a_cont-dist_b_cont))
