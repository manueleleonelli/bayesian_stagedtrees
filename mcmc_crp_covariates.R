...
mcmc_crp_distances <- function(tree, data, n_save, n_burn = 0, thin = 1, a = 1, kappa = 1, csi = 1, eta, prior = "Friedman", scope = NULL, beta = NULL, tau = NULL, update_SM = TRUE, update_CRP= TRUE){
  ...
  for(iv in 1:n_v){
    ...
    for(ind in 1:n_nums){
      ...
      for(i1 in 1:(N_v_iv-1)){
        for(i2 in (i1+1):N_v_iv){
          mat[i1,i2] <- (means[[i1]][ind]-means[[i2]][ind])^2
          mat[i2,i1] <- mat[i1,i2]
        }
      }
    ...
    for(iv in 1:n_v){
      ...
      if(update_CRP){
        for(i in 1:N_iv){
          ...
          w <- c(kappa, nj_iv * exp(-csi * dist_H_j - Reduce(`+`, Map(function(x, y) x * y, dist_nums_j, eta)) ))
      ...
      if(update_SM){
        ...
        if(stage_1 != stage_2){ # MERGE MOVE
          ...
          log_ar_SM <- log_ar_SM  - csi*(dist_SM - dist_a - dist_b) - sum(eta*(dist_SM_cont-dist_a_cont-dist_b_cont))
